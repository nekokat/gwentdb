import sqlite3
import toml
from table import create, count, header
...
def create_pivot(table):
  ...
  if table in ['win_loss', 'versus']:
    ...
    for frkey in fraction.keys():
      column_count = tuple(count([('fraction', frkey),(column, colkey)]) for colkey in table_header.keys())
      row = (frkey,) + column_count
      request = f"INSERT INTO {table} ({header(table)}) VALUES {row}"
      cursor.execute(request)
  conn.commit()

def update_pivot(row, table):
